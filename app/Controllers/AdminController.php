...
class AdminController {
    ...
    public function tiltasToggle($vendeg_id) {
        ...
        try {
            ...
            $vStmt = $this->pdo->prepare("SELECT nev, email FROM vendeg WHERE vendeg_id = ?");
            $vStmt->execute([$vendeg_id]);
            $v = $vStmt->fetch(PDO::FETCH_ASSOC);

            if ($v) {
                if ($ujAllapot === 1) {
                    Mailer::sendTiltasErtesito($v['email'], $v['nev']);
                } else {
                    Mailer::sendFeloldasErtesito($v['email'], $v['nev']);
                }
            }
    ...
    public function asztalPozicio($asztal_id) {
        $this->checkAdminToken();
        ...
        if ($x === null || $y === null) {
            Response::error("Hiányzó pozíció adatok", 400);
            return;
        }

        try {
            $stmt = $this->pdo->prepare("UPDATE asztal SET x_pozicio = ?, y_pozicio = ? WHERE asztal_id = ?");
            $stmt->execute([$x, $y, $asztal_id]);
            Response::success([], "Pozíció mentve.");
        } catch (Exception $e) {
            Response::error("Hiba a pozíció mentésekor");
        }
    }
}
